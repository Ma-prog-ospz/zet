...
def create_parser():
    ...
    add("--realtime-url", help="URL to fetch the GTFS protobuf file", default="https://www.zet.hr/gtfs-rt-protobuf")
    add("--static-url", help="URL to fetch the GTFS static data", default="https://www.zet.hr/gtfs-scheduled/latest")
    add("--realtime-dt", type=float, default=10, help="Interval between realtime fetches (seconds)")
    add("--static-dt", type=float, default=3600, help="Interval between static fetches (seconds)")
    add("--dir", default=".", help="Directory to store snapshots")
    add("--port", type=int, default=5000, help="Flask server port (default: 5000)")
    return parser
...
def main():
    ...
    app.run(host="0.0.0.0", port=port, debug=True)  # Pokreni Flask server s debugom
